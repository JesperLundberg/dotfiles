	...
	statusline.setup({
		...
		content = {
			active = function()
				...
				local right_groups = diagnostic_groups()

				vim.list_extend(right_groups, {
					{
						hl = "MiniStatuslineDevinfo",
						strings = { get_lsps() },
					},
				...
				vim.list_extend(groups, left_groups)

				-- This is the point where Vim is allowed to truncate.
				table.insert(groups, "%<")

				table.insert(groups, {
					hl = "MiniStatuslineFilename",
					strings = { cwd_relative_path() },
				})

				table.insert(groups, "%=")

				vim.list_extend(groups, right_groups)
